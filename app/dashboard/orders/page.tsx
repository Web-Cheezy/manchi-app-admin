...
import { extractOrderIdFromMetadata, fetchPaidOrders, fetchVerifiedTransactions } from '@/utils/paidOrders'
  ...
  async function fetchOrders() {
    ...
    try {
      const [rows, transactions] = await Promise.all([
        fetchPaidOrders(currentAdmin),
        fetchVerifiedTransactions(currentAdmin),
      ])
      ...
      >()
      const emailByOrderId = new Map<number, string>()

      for (const tx of transactions) {
        const orderId = extractOrderIdFromMetadata(tx.metadata)
        if (orderId !== null && tx.email && !emailByOrderId.has(orderId)) {
          emailByOrderId.set(orderId, tx.email)
        }
      }
      ...
      const normalized: AdminOrder[] = rows.map((row) => ({
        ...
        profiles: {
          ...
          email: emailByOrderId.get(row.id) ?? null,
        },
      }))
        ...
        <CardBody className="p-0">
          ...
          <table className="min-w-225 w-full text-sm text-left">
            ...
            <tbody className="divide-y divide-zinc-50">
              {loading ? (
                <tr>
                  <td colSpan={9} className="px-6 py-10 text-center text-sm text-zinc-500">Loading…</td>
                </tr>
              ) : filteredOrders.length === 0 ? (
                <tr>
                  <td colSpan={9} className="px-6 py-10 text-center text-sm text-zinc-500">No orders found.</td>
                </tr>
              ) : (
